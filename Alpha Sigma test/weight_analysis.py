...
and T-alpha gating for all 3 adaptive layers.
"""
...
def combined_activation_np(z, weights, c, d, b, a=A):
    ...
    bases = np.stack([
        np.broadcast_to(z_, (M, len(z))),  # z
        np.broadcast_to(z_ ** 2, (M, len(z))),  # z²
        np.broadcast_to(np.cos(z_), (M, len(z))),  # cos(z)
        np.broadcast_to(np.sin(z_), (M, len(z))),  # sin(z)
        ta * z_,                            # talpha(z)*z
    ], axis=-1)                             # (M, N, 5)
...
# ── Figure 1: Adaptive Layer Overview (3×3) ──────────────────────────
...
for row, (layer, name) in enumerate(zip(layers, LAYER_NAMES)):
    ...
    ax = axes1[row, 1]
    activations = combined_activation_np(X, w, c, d, b)  # (64, N)
    for i in range(64):
...
# ── Figure 2: Learned T-alpha Gating (1×3) ───────────────────────────
...
for col, (layer, name) in enumerate(zip(layers, LAYER_NAMES)):
    ax = axes2[col]
    c, d, b = layer["c"], layer["d"], layer["b"]
    ta_z = talpha_np(X, c, d, b) * X[np.newaxis, :]  # (64, N)
