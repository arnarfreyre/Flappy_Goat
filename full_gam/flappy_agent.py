...
class FlappyAgent():
    ...
    def run_training(self, gamma, lam, clip_eps, value_coef, entropy_coef, max_grad_norm,
                     learning_rate, ppo_epochs, num_epochs, games_per_epoch=20, minibatch_size=128, print_freq=100):
        ...
        for epoch in range(num_epochs):
            ...
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = lr
            # Collect N fresh games with the current policy
            batch = []
            for _ in range(games_per_epoch):
                self.play_episode(episode, mode='Explore')
                batch.append(self.memory)
                total_rewards.append(self.memory[:, 9].sum().item())
            ...
            if (epoch + 1) % print_freq == 0:
                recent = total_rewards[-(print_freq * games_per_epoch):]
                avg_pipes = sum(recent) / len(recent) + 5
